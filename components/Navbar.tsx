...
const Navbar: React.FC<NavbarProps> = ({ isDarkMode, toggleTheme }) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled || isMenuOpen ? 'bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-b border-slate-200/50 dark:border-white/5 py-3 shadow-[0_2px_20px_rgba(0,0,0,0.02)]' : 'bg-transparent py-6'}`}>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-white dark:bg-slate-950 md:hidden transition-all duration-500 ease-in-out ${isMenuOpen ? 'opacity-100 pointer-events-auto translate-y-0' : 'opacity-0 pointer-events-none -translate-y-full'}`}>
        <div className="flex flex-col items-center justify-center h-full space-y-8 px-6 pt-20">
          {navLinks.map((link, idx) => (
            <Link
              ...
              className={`text-2xl font-black uppercase tracking-tighter transition-all ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'} ${location.pathname === link.path ? 'text-blue-600' : 'text-slate-400 dark:text-slate-500'}`}
            >
              {link.name}
            </Link>
          ))}
          <div className={`pt-8 border-t border-slate-100 dark:border-slate-800 w-full text-center transition-all delay-300 ${isMenuOpen ? 'opacity-100' : 'opacity-0'}`}>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Team S.C.A.A.M Mission Genesis</p>
          </div>
        </div>
      </div>
    </nav>
