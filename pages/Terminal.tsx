...
const DATA_SOURCES = [
  { name: "MINESEC OFFICIAL STATISTICS", url: "https://www.minesec.gov.cm", icon: "fa-building-columns" },
  { name: "NATIONAL INSTITUTE OF STATISTICS (INS)", url: "https://www.ins-cameroun.cm", icon: "fa-chart-pie" },
  { name: "UNESCO UIS CAMEROON PROFILE", url: "http://uis.unesco.org/en/country/cm", icon: "fa-globe" },
  { name: "WORLD BANK EDUCATION DATA", url: "https://data.worldbank.org/country/cameroon", icon: "fa-database" }
];
...
const Terminal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const messages = [
      "INITIALIZING MISSION GENESIS CORE...",
  ...
  const processCommand = (e: React.FormEvent) => {
    ...
    switch (cmd) {
      case 'help':
        response = "AVAILABLE COMMANDS: STATUS, CLEAR, SCAN, LAUNCH, STATS, WHOAMI, EXIT";
        break;
      case 'status':
        response = `SYSTEM STABLE. UPTIME: ${systemUptime}. LOGIC ACCURACY: 98.4%. NODES: 3 ACTIVE.`;
        break;
      case 'clear':
      ...
      case 'stats':
      case 'data':
        response = "FETCHING EXTERNAL UPLINKS: [MINESEC], [INS], [UNESCO], [WORLD BANK]. CLICK DASHBOARD REPOSITORY FOR ACCESS.";
        break;
      case 'whoami':
        response = "OPERATOR ROLE: MISSION GENESIS STRATEGIST. ACCESS LEVEL: ALPHA.";
        break;
      case 'exit':
        response = "REDIRECTING TO DATA HUB INTERFACE...";
        setTimeout(() => window.location.hash = "/data-hub", 1500);
        break;
      default:
  ...
  if (bootSequence) {
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center z-[200] font-mono p-8">
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white pt-24 pb-32 font-sans selection:bg-blue-600 selection:text-white overflow-x-hidden">
      ...
      {moduleLoading && (
        <div className="fixed inset-0 z-[300] bg-black/90 backdrop-blur-xl flex items-center justify-center p-8">
           <div className="max-w-md w-full text-center">
              ...
              <p className="text-gray-500 text-xs mb-8 uppercase tracking-widest font-bold">Synchronizing Simulation Parameters for National-Skill-Audit</p>
      ...
      {activeModule && (
        <div className="fixed inset-0 z-[250] bg-gray-950 flex flex-col p-4 md:p-8 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 animate-fade-in">
          <div className="text-left">
            <div className="inline-flex items-center space-x-2 px-3 py-1 bg-blue-600/10 border border-blue-500/20 rounded-full mb-4">
              ...
              <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Active Session: National-Think-Tank</span>
            </div>
            <h1 className="text-5xl font-black tracking-tighter text-white uppercase text-left">S.C.A.A.M <span className="text-blue-500">TERMINAL</span></h1>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
            ...
            <div className="text-right">
              <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Logic Accuracy</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          <div className="lg:col-span-8 space-y-8 animate-fade-in" style={{ animationDelay: '0.2s' }}>
            <div className="bg-white/[0.03] border border-white/10 rounded-[40px] p-8 md:p-12 relative overflow-hidden group">
              ...
              <div className="inline-block border-2 border-dashed border-blue-500/50 rounded-2xl p-4 mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-6 text-left">
                  {[
                    ...
                    { label: "Applied Engineering", value: 18, status: "Critical Lack", color: "orange" }
                  ].map((skill) => (
                    <div key={skill.label}>
                ...
                <div className="relative aspect-square flex items-center justify-center border-4 border-dashed border-blue-500/40 rounded-3xl p-8 m-2 group-hover:border-blue-500/60 transition-colors">
                  <div className="absolute inset-4 border border-red-500/20 rounded-full animate-[spin_20s_linear_infinite]"></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white/[0.03] border border-white/10 rounded-[40px] p-8 text-left">
                <h4 className="text-[11px] font-black text-blue-500 uppercase tracking-widest mb-6">Active Deployments</h4>
                <div className="space-y-4">
                  {[
                    { node: "BUEA-CENTRAL", task: "Mission Genesis Pilot", status: "ACTIVE" },
                    { node: "COASTAL-NODE", task: "Impact Redundancy", status: "SYNCING" }
                  ].map((item) => (
                    <div key={item.node} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
                      <div>
                        <p className="text-xs font-black text-white">{item.node}</p>
                        <p className="text-[10px] font-medium text-gray-500">{item.task}</p>
                      </div>
                      <span className={`text-[9px] font-black px-2 py-1 rounded bg-${item.status === 'STABLE' ? 'emerald' : 'blue'}-500/20 text-${item.status === 'STABLE' ? 'emerald' : 'blue'}-500`}>
                        {item.status}
                      </span>
                    </div>
                  ))}
              ...
              <div className="bg-white/[0.03] border border-white/10 rounded-[40px] p-8 flex flex-col justify-center items-center text-center">
                ...
                <h4 className="text-lg font-black text-white mb-2">New Quest Ready</h4>
                <p className="text-sm text-gray-500 mb-6 font-medium">Scenario: Structural failure risk detected. Deploy national engineering protocols.</p>
                <button 
                  onClick={handleLaunchModule}
                  className="w-full py-3 bg-blue-600 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-blue-500 transition-all active:scale-95 shadow-xl shadow-blue-600/20"
                >
                  Launch Module
                </button>
              </div>
          ...
          <div className="lg:col-span-4 space-y-8 animate-fade-in text-left" style={{ animationDelay: '0.4s' }}>
            <div className="bg-gradient-to-br from-blue-600 to-blue-900 rounded-[40px] p-8 text-white relative overflow-hidden group">
              ...
              <div className="space-y-8">
                <div>
                  <p className="text-4xl font-black tracking-tighter">70%</p>
                  <p className="text-[10px] font-bold uppercase tracking-widest opacity-80 mt-1">Target Productivity Benchmark</p>
                </div>
                ...
                <div className="flex justify-between items-end">
                  <div>
                    ...
                    <p className="text-[9px] font-bold uppercase tracking-widest opacity-80 mt-1 text-left">Current National Baseline</p>
                  </div>
                  <div className="text-right">
            ...
            <div className="bg-white/[0.03] border border-white/10 rounded-[40px] p-8 group hover:bg-white/[0.05] transition-all">
              <h4 className="text-[11px] font-black text-emerald-500 uppercase tracking-widest mb-6 flex items-center">
              ...
              <div className="space-y-4">
                {DATA_SOURCES.map((source) => (
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="flex items-center space-x-4 p-3 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/5 hover:border-blue-500/30 transition-all"
                  >
                    ...
                    <div className="flex-grow">
                      <p className="text-[10px] font-black text-white uppercase tracking-tight">{source.name}</p>
                    </div>
            ...
            <div className="bg-white/[0.03] border border-white/10 rounded-[40px] p-8">
              <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-6">Strategic Nodes</h4>
              <div className="space-y-6">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-grow">
                    <p className="text-xs font-black">Cameroon Ed-Network</p>
                    <p className="text-[10px] text-gray-500">National Think-Tank Coverage</p>
                  </div>
                </div>
              </div>
            </div>

            <Link to="/data-hub" className="block w-full py-6 bg-white/[0.03] border border-white/10 rounded-[40px] text-center text-[11px] font-black uppercase tracking-[0.3em] hover:bg-white hover:text-black transition-all">
              Return to Data Hub
            </Link>
          </div>
        </div>

        {/* HUD Command Terminal */}
        <div className="fixed bottom-0 left-0 w-full bg-black/80 backdrop-blur-md border-t border-white/10 p-4 md:px-8 z-[150] animate-in slide-in-from-bottom duration-500">
              ...
              <div className="flex-grow w-full">
                 <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden h-24 md:h-16 flex flex-col md:flex-row items-stretch">
                    ...
                    <form onSubmit={processCommand} className="flex items-center border-t md:border-t-0 md:border-l border-white/10 bg-black/40 px-4 group">
                       <span className="text-blue-500 font-mono text-sm mr-3 font-black group-focus-within:animate-pulse">_SYS:</span>
                       <input 
                          type="text" 
                          value={commandInput}
                          onChange={(e) => setCommandInput(e.target.value)}
                          placeholder="ENTER PROTOCOL (TRY 'STATS')..." 
